...
class AnimatedQuat {
	...
	void read(ByteBuffer buf) throws IOException {
		...
		if (type == 2) {
			...
			for (int i = 0; i < count; i++) {
				dataIn[i] = new Quat4f(buf.getFloat(), buf.getFloat(), buf
						.getFloat(), buf.getFloat());
				dataOut[i] = new Quat4f(buf.getFloat(), buf.getFloat(), buf
						.getFloat(), buf.getFloat());
			}
			...
			for (int i = 0; i < count; i++)
				data[i] = new Quat4f(buf.getFloat(), buf.getFloat(), buf
						.getFloat(), buf.getFloat());
