...
class ModelLoader {
	...
	static boolean load(Model model, ByteBuffer buf, boolean loadTextures,
			String path) {
		try {
			...
			if (ofsVertices != 0 && ofsIndices != 0 && ofsMaterials != 0
					&& ofsMeshes != 0 && ofsTexGroups != 0
					&& ofsBakedTextures != 0 && ofsBones != 0
					&& ofsSlotBones != 0 && ofsSkinColors != 0
					&& ofsFacialHairs != 0)
				if (ofsHairStyles == 0)
					;
			...
			if (numMaterials > 0) {
				...
				for (int i = 0; i < numMaterials; i++) {
					...
					if (loadTextures && mat.mFilename.length() > 0) {
						FileLoader fl = new FileLoader(1, model, mat,
								("textures/" + mat.mFilename).toLowerCase());
						fl.start();
					}
					materials[i] = mat;
				}
			...
			if (VersionNumber >= 500) {
				...
				if (VersionNumber == 500) {
					...
					loadAnims(model, buf);
				} else if (VersionNumber >= 510) {
					model.mNumLoopAnims = numLoopAnims;
					model.mNumActiveAnims = numActiveAnims;
					FileLoader fl = new FileLoader(1005, model, (path
							.substring(0, path.length() - 3) + "anm")
							.toLowerCase());
					fl.start();
				}
			}
	...
	static boolean loadAnims(Model model, ByteBuffer buf) throws IOException {
		...
		model.mCurrentAnim = model.mLoopAnims[0];
		return true;
	}
}
